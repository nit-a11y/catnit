...
export class GameEngine {
  ...
  private camera = {
    ...
    targetX: 0,
    targetY: 0,
    smoothFactor: 0.15 // Aumentado para movimentação mais suave
  };
  
  // Sistema de pooling para partículas
  private particlePool: Particle[] = [];
  private maxParticles = 100; // Limite máximo de partículas
  ...
  update(dt: number) {
    ...
    this.particles = this.particles.filter(p => { 
      p.update(dt); 
      if (p.life <= 0) {
        // Retornar partícula ao pool para reutilização
  ...
  private updateCamera() {
    ...
    // Aplicar smooth follow com fator dinâmico baseado na velocidade do jogador
    const playerSpeed = Math.sqrt(this.player.vx * this.player.vx + this.player.vy * this.player.vy);
    const dynamicSmoothFactor = Math.min(0.25, 0.08 + playerSpeed * 0.02);
    
    // Usar LERP para movimento mais suave
    this.camera.x += (targetX - this.camera.x) * dynamicSmoothFactor;
    this.camera.y += (targetY - this.camera.y) * dynamicSmoothFactor;
  ...
  private getParticleFromPool(x: number, y: number, color: string): Particle {
    // Obter partícula do pool ou criar nova se necessário
  ...
  private createExplosion(x: number, y: number, color: string, count = 8) {
    ...
    for (let i = 0; i < actualCount; i++) {
      ...
      const particle = this.getParticleFromPool(x, y, color);
      particle.size = 2 + Math.random() * 6; // Reduzido para performance
      particle.decay = 0.002 + Math.random() * 0.004; // Aumentado para desaparecer mais rápido
      this.particles.push(particle);
    }
    ...
    if (count > 12) {
      for (let i = 0; i < 2; i++) { // Reduzido de 4 para 2
        ...
        const lightParticle = this.getParticleFromPool(x, y, '#ffffff');
        lightParticle.size = 1 + Math.random() * 2;
        lightParticle.decay = 0.003;
  ...
  private draw() {
    ...
    // Desenhar grid do mundo otimizado
    this.ctx.strokeStyle = '#2d1e3e';
    this.ctx.lineWidth = 1;
    
    // Otimizar cálculo do grid - renderizar apenas linhas visíveis
    const gridSize = 40;
    ...
    // Desenhar linhas verticais otimizadas
    this.ctx.beginPath();
    ...
    // Desenhar linhas horizontais otimizadas
    this.ctx.beginPath();
...
class Player {
  x: number; y: number;
  radius = 20; speed = 0.2;
  vx = 0; vy = 0; // Propriedades de velocidade para otimização
  hp = 100; maxHp = 100;
  mp = 100; maxMp = 100;
  manaRegen = 5;
  ...
  update(dx: number, dy: number, dt: number, cw: number, ch: number) {
    if (this.dashTimer > 0) {
      ...
      if (mag > 0) {
        ...
        // Registrar velocidade para otimização da câmera
        this.vx = (dx / mag) * currentSpeed;
        this.vy = (dy / mag) * currentSpeed;
      } else {
        this.vx = 0;
        this.vy = 0;
      }
