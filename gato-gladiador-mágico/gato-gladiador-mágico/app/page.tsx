...
export default function Home() {
  ...
  return (
    <main className={`h-screen w-screen grid ${isMobile ? 'grid-cols-1 grid-rows-[50px_1fr]' : 'grid-cols-[260px_1fr_260px] grid-rows-[100px_1fr_140px]'} gap-3 p-3 bg-[#1a1423] overflow-hidden`}>
      {/* HEADER */}
      ...
      <div className="relative overflow-hidden group border-[4px] md:border-[6px] border-[#3d3051] bg-[#1a1423] touch-none">
        ...
        {/* MOBILE HUD - TOP BAR */}
        {isMobile && (
          <div className="absolute top-0 left-0 right-0 p-2 z-[50] flex flex-col items-center gap-1 pointer-events-none">
            {/* Health & XP Cluster */}
            <div className="w-full max-w-[280px] flex flex-col gap-1">
              {/* HP Bar */}
              <div className="h-5 w-full bg-black/40 border-2 border-[#ff4d6d]/30 overflow-hidden relative rounded-full shadow-[0_0_12px_rgba(255,77,109,0.2)]">
                <motion.div 
                  initial={false}
                ...
                <div className="absolute inset-0 flex items-center justify-center text-[8px] font-black text-white px-1 drop-shadow-md">
                  {Math.ceil(stats.hp)}/{stats.maxHp}
                </div>
              </div>
              
              {/* XP Bar */}
              <div className="h-1 w-full bg-black/40 overflow-hidden relative rounded-full">
                <motion.div 
              ...
              {/* MP Bar */}
              <div className="h-3 w-full bg-black/40 border border-[#a388ee]/30 overflow-hidden relative rounded-full">
                <motion.div 
                  animate={{ width: `${(stats.mp / stats.maxMp) * 100}%` }}
                  className="h-full bg-gradient-to-r from-[#a388ee] to-[#bdb2ff]"
                />
                <div className="absolute inset-0 flex items-center justify-center text-[7px] font-black text-white px-1 uppercase italic">
                  {Math.ceil(stats.mp)}/{stats.maxMp}
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-2 items-center">
              ...
              <span className="text-[7px] font-black text-white uppercase tracking-tighter">Dodge</span>
              {/* Cooldown Spinner */}
              {(stats as any).cooldowns?.dash > 0 && (
                <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                  <span className="text-xs font-bold text-[#ffcc33]">{Math.ceil((stats as any).cooldowns.dash / 1000)}s</span>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-1">
              <button 
                onTouchStart={(e) => { e.preventDefault(); engineRef.current?.executeAction('shoot'); }}
                className="w-14 h-14 rounded-full bg-gradient-to-br from-[#a388ee] to-[#6366f1] border-3 border-white/40 flex flex-col items-center justify-center shadow-[0_3px_12px_rgba(163,136,238,0.4)] active:scale-90 active:brightness-125 transition-all z-20 group relative overflow-hidden"
              >
                <Target size={20} className="text-white drop-shadow-md" />
                <span className="text-[7px] font-black text-white uppercase tracking-tighter">Atk</span>
                {/* Cooldown Spinner */}
                {(stats as any).cooldowns?.shoot > 0 && (
                  <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                    <span className="text-xs font-bold text-[#ffcc33]">{Math.ceil((stats as any).cooldowns.shoot / 1000)}s</span>
                  </div>
                )}
              </button>
              ...
              <button 
                ...
                className="w-14 h-14 rounded-full bg-gradient-to-br from-[#ff4d6d] to-[#ff758f] border-3 border-white/40 flex flex-col items-center justify-center shadow-[0_3px_12px_rgba(255,77,109,0.4)] active:scale-90 active:brightness-125 transition-all z-20 group relative overflow-hidden"
              >
                <Zap size={20} className="text-white drop-shadow-md" />
                <span className="text-[7px] font-black text-white uppercase tracking-tighter">Esp</span>
                {/* Cooldown Spinner */}
                {(stats as any).cooldowns?.special > 0 && (
                  <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                    <span className="text-xs font-bold text-[#ffcc33]">{Math.ceil((stats as any).cooldowns.special / 1000)}s</span>
                  </div>
                )}
              </button>
            </div>
